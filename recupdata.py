...
def run(token, date, ville):
    url2 = 'https://ressources.data.sncf.com/api/records/1.0/search/?dataset=referentiel-gares-voyageurs&q=&sort=gare_alias_libelle_noncontraint&facet=gare_alias_libelle_noncontraint&refine.gare_alias_libelle_noncontraint='+ville

    rep2 = requests.get(
    url2,
    headers={'Authorization': token})
    villeid = rep2.json()['records'][0]['fields']['uic_code'][2:]



    start_date = date.strftime("%Y%m%dT%H%M%S")
    end_date = (date + timedelta(days=1)).strftime("%Y%m%dT%H%M%S")
    url = 'https://api.navitia.io/v1/coverage/sncf/stop_areas/stop_area%3ASNCF%3A'+villeid+'/physical_modes/physical_mode%3ALongDistanceTrain/arrivals?from_datetime='+start_date+'&until_datetime='+end_date+'&count=1000&'
